...
df = pd.read_csv("iterable_subscription_test.csv")
# print(df.values)
...
for x in df.values:
    ...
    if x[2] != "":
        if x[2].lower() == "subscribed":
            ...
            print(f"Row {n}: sms channel: {x[2]} Response: ", sms_channel_response.json())
            ...
            print(f"Row {n}: sms type: {x[2]} Response: ", sms_type_response.json())

            # print(f"Row {n}: SMS Subscribe - {sms_channel_url}; {sms_type_url}")

        elif x[2].lower() == "pending":
            sms_channel_url = subscription_url(iterable_subscribe, channel_name, sms_channel_id, x[0])
            sms_channel_response = requests.patch(url=sms_channel_url, headers=api_headers)
            print(f"Row {n}: sms channel: {x[2]} Response: ", sms_channel_response.json())
            ...
            print(f"Row {n}: sms type: {x[2]} Response: ", sms_unsub_response.json())
